...
function render(){
 const result=MoeSearch.sort(MoeSearch.rows(ledger,$('#query').value,$('#filter').value),sortKey,sortDirection);
 ...
 for(const r of result){
  ...
  const pages=[...r.pageDates.entries()].sort((a,b)=>a[0]-b[0]);
  const name=node('td',r.name);name.append(node('small',pages.map(([p])=>pageName(p)).join(' / ')));
  const owner=node('td',r.account.name);owner.append(node('small',r.account.loginId||'ID未登録'));
  ...
  if(pages.length>1){
   ...
   details.append(node('summary','ページ別日時'));
   for(const [p,stamp] of pages)details.append(node('small',`${pageName(p)}：${dateLabel(stamp)}`));
